...
def smart_resize(
    ...
    """
    强制单 Tile 策略：将图像缩放到接近 224x224 的总像素量 (50176)。
    同时保持原图比例，并对齐到 factor (patch_size * merge_size)。
    """
    # 1. 设定目标总像素 (224 * 224 = 50176)
    TARGET_PIXELS = 224 * 224 
    
    # 2. 计算缩放比例 beta，以保持宽高比
    beta = math.sqrt(TARGET_PIXELS / (height * width))
    
    # 3. 计算初步缩放后的尺寸
    h_bar = height * beta
    w_bar = width * beta
    
    # 4. 强制对齐到 factor (Qwen2-VL 通常为 28)
    # 确保 resize 后的尺寸能被 patch 和 merge 逻辑整除
    h_bar = max(round(h_bar / factor), 1) * factor
    w_bar = max(round(w_bar / factor), 1) * factor
    
    # [打印日志用于调试]
    # print(f"Original: {height}x{width} -> Resized: {h_bar}x{w_bar}")
